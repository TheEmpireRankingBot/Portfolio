...
(function () {
    ...
    const extras = [
      {
        ...
        category: 'food', area: city.neighborhoods[1].name,
        duration: '3 hr', rating: 4.8,
        reviewCount: range(rng, 200, 2400),
        price: Math.round(85 * mult), currency: 'USD',
        img: ACT_IMGS.food, booksFast: true,
      },
      {
        id: `${city.id}-an`,
  ...
  function generateReviews(city) {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      out.push({ id: `${city.id}-r${i}`, author, rating, text: tpl(city), date: `${monthsAgo} mo ago · ${stayed}` });
    }
  ...
  function summary(city, h, a, f) {
    ...
    return { hotelFrom: minHotel, flightFrom: minFlight, avgActivity, total3DayFrom: minFlight + minHotel * 3 + avgActivity * 4 };
  }
    ...
    fetchCity(cityId) {
      ...
      return new Promise((resolve) => {
        ...
        setTimeout(() => {
          ...
          const data = { cityId, hotels, activities, flights, reviews, summary: summary(city, hotels, activities, flights), fetchedAt: Date.now() };
          cache[cityId] = data;
          resolve(data);
        }, delay);
      });
