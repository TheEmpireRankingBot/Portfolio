...
function CityDetail({ city, onBack, onWish, wished }) {
  ...
  return (
    <div ref={scrollRef} style={{ position: 'absolute', inset: 0, background: SURFACE, overflow: 'auto', overscrollBehavior: 'contain' }}>
      ...
      <div style={{
        position: 'sticky', top: 0, zIndex: 15, background: SURFACE,
        borderBottom: `1px solid ${BORDER}`,
        display: 'flex', gap: 0, overflowX: 'auto', scrollbarWidth: 'none', padding: '4px 8px',
      }}>
        {[
      ...
      <div style={{ padding: '18px 22px 140px' }}>
        ...
        <Section label="Need to know"><PracticalInfo p={city.practical} /></Section>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {itinerary.map((d) => (
              <div key={d.day} style={glass({ borderRadius: 18, overflow: 'hidden' })}>
                <div style={{ padding: '12px 16px 10px', borderBottom: `1px solid ${BORDER}`, display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                ...
                {Object.entries(d.slots).map(([time, act]) => (
                  <div key={time} style={{ padding: '10px 16px', borderBottom: time === 'Evening' ? 'none' : `1px solid ${BORDER}`, display: 'flex', gap: 12, alignItems: 'flex-start' }}>
        ...
        {db && (
          <div style={{ ...glass({ borderRadius: 20 }), padding: '18px 20px', marginTop: 4, marginBottom: 20, background: 'linear-gradient(160deg, rgba(244,217,166,0.10), rgba(244,217,166,0.03))', border: '1px solid rgba(244,217,166,0.22)' }}>
        ...
        <button onClick={() => onWish(city)} style={{
          ...ctaPrimary,
          background: wished ? 'linear-gradient(180deg, #F8E2B4 0%, #E0B86E 100%)' : 'linear-gradient(180deg, rgba(255,255,255,0.10), rgba(255,255,255,0.05))',
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <ExternalLink href={mkFlightURL('', city.name)} style={{ flex: 1, padding: '14px 0', textAlign: 'center', ...glass({ borderRadius: 14 }), fontFamily: FONT_SANS, fontSize: 13, fontWeight: 600, color: FG }}>✈ Find flights</ExternalLink>
          <ExternalLink href={mkBookingURL(city.name)} style={{ flex: 1, padding: '14px 0', textAlign: 'center', ...glass({ borderRadius: 14 }), fontFamily: FONT_SANS, fontSize: 13, fontWeight: 600, color: FG }}>🏨 Book hotel</ExternalLink>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: SURFACE, overflow: 'auto' }}>
      <div style={{ padding: '68px 22px 20px', background: 'linear-gradient(180deg, rgba(18,26,55,0.55), transparent)' }}>
        <div style={{ fontFamily: FONT_SANS, fontSize: 10.5, letterSpacing: 2.8, textTransform: 'uppercase', color: GOLD_DIM, marginBottom: 6 }}>Your collection</div>
        <div style={{ fontFamily: FONT_DISPLAY, fontSize: 46, lineHeight: 1, fontWeight: 500, color: FG, letterSpacing: -1.2 }}>Wishlist</div>
        <div style={{ fontFamily: FONT_SANS, fontSize: 13, color: FG_MUTED, marginTop: 8 }}>
          {list.length === 0 ? 'Nothing saved yet — spin the globe and tap ♡' : `${list.length} ${list.length === 1 ? 'destination' : 'destinations'} · tap to plan`}
        </div>
      </div>
      {list.length === 0 && (
        <div style={{ margin: '0 16px 24px', ...glass({ borderRadius: 22, padding: '48px 24px' }), textAlign: 'center' }}>
          ...
          <div style={{ fontFamily: FONT_DISPLAY, fontStyle: 'italic', fontSize: 20, color: FG_DIM, lineHeight: 1.4 }}>Every trip starts with a wish.</div>
          <div style={{ fontFamily: FONT_SANS, fontSize: 13, color: FG_MUTED, marginTop: 8 }}>Tap any city on the globe and hit ♡ to save it here.</div>
        </div>
      )}
      <div style={{ padding: '4px 16px 148px', display: 'flex', flexDirection: 'column', gap: 14 }}>
        {list.map(c => (
          <div key={c.id} style={{ position: 'relative' }}>
            <button onClick={() => onTap(c)} style={{ width: '100%', padding: 0, border: 'none', cursor: 'pointer', borderRadius: 22, overflow: 'hidden', height: 195, background: '#000', textAlign: 'left', display: 'block' }}>
              ...
              <div style={{ position: 'absolute', left: 18, right: 18, bottom: 16 }}>
                <div style={{ fontFamily: FONT_SANS, fontSize: 10, letterSpacing: 2.2, textTransform: 'uppercase', color: GOLD_DIM, marginBottom: 3 }}>{c.flag} {c.country}</div>
            ...
            <button onClick={() => onRemove(c)} style={{ position: 'absolute', top: 14, right: 14, width: 32, height: 32, borderRadius: 16, background: 'rgba(0,0,0,0.55)', backdropFilter: 'blur(8px)', border: '1px solid rgba(255,255,255,0.15)', color: GOLD, fontSize: 15, cursor: 'pointer' }}>♥</button>
          </div>
        ))}
      </div>
    </div>
...
function SurpriseOverlay({ phase, city }) {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', zIndex: 25, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column' }}>
      <div style={{ position: 'absolute', inset: 0, background: phase === 'spinning' ? 'radial-gradient(circle, rgba(0,0,0,0) 30%, rgba(0,0,0,0.58) 100%)' : 'radial-gradient(circle, rgba(244,217,166,0.08) 0%, rgba(0,0,0,0.62) 80%)', transition: 'background 600ms' }} />
      {phase === 'spinning' && (
        <div style={{ position: 'relative', textAlign: 'center', marginTop: -80 }}>
          <div style={{ fontFamily: FONT_SANS, fontSize: 10, letterSpacing: 3.5, textTransform: 'uppercase', color: GOLD_DIM, marginBottom: 12 }}>Searching the world</div>
          <div style={{ fontFamily: FONT_DISPLAY, fontSize: 26, fontStyle: 'italic', color: 'rgba(255,255,255,0.90)', letterSpacing: -0.3 }}>where shall we send you?</div>
      ...
      {phase === 'revealing' && city && (
        <div style={{ textAlign: 'center', marginTop: -110, animation: 'revealIn 700ms cubic-bezier(0.22,1,0.36,1) both' }}>
          ...
          <div style={{ fontFamily: FONT_DISPLAY, fontSize: 62, lineHeight: 1, fontWeight: 500, color: FG, letterSpacing: -1.8, textShadow: '0 4px 40px rgba(244,217,166,0.4)' }}>{city.name}</div>
          <div style={{ fontFamily: FONT_SANS, fontSize: 12, letterSpacing: 2.6, textTransform: 'uppercase', color: 'rgba(255,255,255,0.68)', marginTop: 10 }}>{city.flag} {city.country}</div>
        </div>
      )}
    </div>
  );
...
function SurpriseFab({ onTap, hidden }) {
  if (hidden) return null;
  return (
    <button onClick={onTap} style={{ position: 'absolute', left: '50%', bottom: 110, transform: 'translateX(-50%)', padding: '14px 24px 14px 20px', borderRadius: 100, background: 'linear-gradient(180deg, rgba(244,217,166,0.96), rgba(220,178,108,0.93))', border: '1px solid rgba(255,230,180,0.55)', boxShadow: '0 8px 32px rgba(244,217,166,0.30), 0 2px 0 rgba(255,255,255,0.28) inset', display: 'inline-flex', alignItems: 'center', gap: 10, fontFamily: FONT_SANS, fontSize: 13, fontWeight: 700, letterSpacing: 0.2, color: '#1A1208', cursor: 'pointer', zIndex: 15 }}>
