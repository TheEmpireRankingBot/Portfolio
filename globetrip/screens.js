      ...
      <div style={glass({
        ...
        <div style={{
          ...
          <div style={{ position: 'absolute', left: 22, right: 22, bottom: 16 }}>
            <div style={{
              ...
              textTransform: 'uppercase', color: 'rgba(244,217,166,0.95)', marginBottom: 2,
            }}>{city.country}</div>
            <div style={{
              fontFamily: FONT_DISPLAY, fontSize: 38, lineHeight: 1.0,
              fontWeight: 500, color: '#fff', letterSpacing: -0.5,
            }}>{city.name}</div>
          </div>
        </div>
        <div style={{ padding: '18px 22px 22px' }}>
          <div style={{
            ...
            color: 'rgba(255,255,255,0.86)', marginBottom: 18,
          }}>"{city.hook}"</div>
...
function CityDetail({ city, onBack, onWish, wished }) {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: '#070B1C', overflow: 'auto', overscrollBehavior: 'contain' }}>
      <div style={{ position: 'relative', height: 380, backgroundImage: `url(${city.hero})`, backgroundSize: 'cover', backgroundPosition: 'center' }}>
        ...
        <button onClick={onBack} style={{ position: 'absolute', top: 60, left: 16, width: 40, height: 40, borderRadius: 20, background: 'rgba(0,0,0,0.45)', backdropFilter: 'blur(14px)', border: '1px solid rgba(255,255,255,0.15)', color: '#fff', fontSize: 18, cursor: 'pointer' }}>‹</button>
        <button onClick={() => onWish(city)} style={{ position: 'absolute', top: 60, right: 16, width: 40, height: 40, borderRadius: 20, background: 'rgba(0,0,0,0.45)', backdropFilter: 'blur(14px)', border: '1px solid rgba(255,255,255,0.15)', color: wished ? '#F4D9A6' : '#fff', fontSize: 16, cursor: 'pointer' }}>{wished ? '♥' : '♡'}</button>
        <div style={{ position: 'absolute', left: 22, right: 22, bottom: 22 }}>
          <div style={{ fontFamily: FONT_SANS, fontSize: 11, letterSpacing: 2.6, textTransform: 'uppercase', color: 'rgba(244,217,166,0.95)', marginBottom: 6 }}>{city.country}</div>
          <div style={{ fontFamily: FONT_DISPLAY, fontSize: 56, lineHeight: 0.95, fontWeight: 500, color: '#fff', letterSpacing: -1.2 }}>{city.name}</div>
          <div style={{ fontFamily: FONT_DISPLAY, fontStyle: 'italic', fontSize: 15, lineHeight: 1.3, color: 'rgba(255,255,255,0.8)', marginTop: 10, maxWidth: 300 }}>"{city.hook}"</div>
        </div>
      </div>
      <div style={{ padding: '22px 22px 140px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8, marginBottom: 26 }}>
          {[{ l: 'Best', v: city.season.split(' · ')[0] }, { l: 'Cost', v: city.cost }, { l: 'Daily', v: city.daily }].map((s,i) => (
        ...
        <Section label="Food to try">
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
            {city.food.map((f, i) => (<div key={i} style={{ ...glass({ borderRadius: 100 }), padding: '8px 14px', fontFamily: FONT_SANS, fontSize: 12, color: '#fff', fontWeight: 500 }}>{f}</div>))}
          </div>
        </Section>
        ...
        <Section label="Weather & best season">
          <div style={{ ...glass({ borderRadius: 16, padding: '16px 18px' }), display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div>
        ...
        <button style={{ width: '100%', padding: '17px 20px', background: 'linear-gradient(180deg, #F8E2B4 0%, #E5BD7A 100%)', border: 'none', borderRadius: 16, fontFamily: FONT_SANS, fontSize: 15, fontWeight: 600, letterSpacing: 0.4, color: '#1A1410', cursor: 'pointer', boxShadow: '0 6px 24px rgba(229,189,122,0.3), inset 0 1px 0 rgba(255,255,255,0.5)', marginTop: 30, marginBottom: 10 }}>Build My Trip</button>
        <div style={{ display: 'flex', gap: 10 }}>
    ...
    <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', zIndex: 25, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column' }}>
      ...
      {phase === 'revealing' && city && (
        <div style={{ textAlign: 'center', marginTop: -120, animation: 'revealIn 700ms cubic-bezier(0.22, 1, 0.36, 1) both' }}>
          ...
          <div style={{ fontFamily: FONT_SANS, fontSize: 12, letterSpacing: 2.4, textTransform: 'uppercase', color: 'rgba(255,255,255,0.7)', marginTop: 10 }}>{city.country}</div>
        </div>
      )}
    </div>
  );
...
function SurpriseFab({ onTap, hidden }) {
  ...
  return (
    <button onClick={onTap} style={{
      ...
      padding: '14px 22px 14px 18px', borderRadius: 100,
