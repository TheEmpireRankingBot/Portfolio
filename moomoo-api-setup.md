...
Installed in virtualenv at `/opt/moomoo-venv` (system pip has broken setuptools — the venv was recreated with `python3 -m venv --upgrade-deps` to fix the previous session's build error; use `/opt/moomoo-venv/bin/python` for moomoo scripts).
